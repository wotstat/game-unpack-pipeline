...
Автоматическая проверка всех поддерживаемых клиентов выполняется каждые два часа на `:23` UTC.
Ручная проверка и прямой ручной запуск обработки остаются доступны через GitHub Actions.
