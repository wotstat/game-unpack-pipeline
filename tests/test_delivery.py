...
def test_download_policy_detects_near_stalls_over_five_minutes() -> None:
    ...
    assert policy.parallel_range_retry_backoff_seconds == 0.25
...
class _FixtureServer(ThreadingHTTPServer):
    ...
    transient_range_ignores_remaining: int
    transient_range_ignore_after: int
    successful_range_responses: int
    range_behavior_lock: threading.Lock
...
class _RangeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if raw_range is not None and not ignore_range:
            with self.server.range_behavior_lock:
                if (
                    self.server.transient_range_ignores_remaining > 0
                    and self.server.successful_range_responses
                    >= self.server.transient_range_ignore_after
                ):
                    self.server.transient_range_ignores_remaining -= 1
                    ignore_range = True
                else:
                    self.server.successful_range_responses += 1
        if raw_range is not None and not ignore_range:
...
def _http_fixture(
    ...
    *,
    ...
    transient_range_ignores: int = 0,
    transient_range_ignore_after: int = 0,
    ...
    server.transient_range_ignores_remaining = transient_range_ignores
    server.transient_range_ignore_after = transient_range_ignore_after
    server.successful_range_responses = 0
    server.range_behavior_lock = threading.Lock()
...
def test_parallel_range_retries_transient_non_partial_response_without_single_stream(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    data = {
    ...
    with _http_fixture(
        ...
        transient_range_ignores=1,
        transient_range_ignore_after=1,
    ) as server:
        plan = _plan(workspace, server)
        monkeypatch.setattr(
            "game_downloader.delivery.urlsplit", lambda value: urlsplit_https(value)
    ...
    assert workspace.blobs.path_for(client.blob_sha256).read_bytes() == data["/client.bin"]
    assert client.transport.parallel_segments == 4
    assert client.transport.parallel_range_fallbacks == ()
    assert len(client_ranges) == 5
    assert all(header is not None for header in client_ranges)
    assert len(set(client_ranges)) == 4
    assert any(
        client.artifact.artifact_id in message
        and "ignored a bounded request with HTTP 200" in message
