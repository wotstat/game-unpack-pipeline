...
`release_name` фиксируется уже из успешно завершённой стадии `resolve`, поэтому сохраняется в
`last_run`, даже если последующая загрузка клиента завершилась ошибкой. `readable_version` читается
из корневого `version.xml` готового snapshot и до получения payload недоступна. Если run завершился
до `resolve`, история страницы явно сообщает, что версия неизвестна.
