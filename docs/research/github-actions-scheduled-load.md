...
Первоначально рассматривался запуск раз в два часа, затем частота была увеличена до одного запуска
в час. После того как scheduled runs не создавались ни при одном варианте, trigger вынесен в
отдельный диагностический workflow `Cron check game releases`. Он временно запускает только
`Hello world` по `*/5 * * * *`, чтобы проверить регистрацию schedule без checker inputs и jobs.
Это тестовая частота, а не production-расписание проверки releases.

Для будущего production-расписания сохраняется выбранная минута `:23`, которая:
...
Это всё ещё эвристика распределения, а не результат публичной поминутной статистики. Текущий
пятиминутный smoke не использует это смещение; `:23` вернётся только после подтверждения scheduler и
переноса реальной автоматической проверки. GitHub также поддерживает явную IANA timezone в
`schedule`.
...
1. Проверить отдельный schedule-only workflow с `Hello world` по `*/5 * * * *`.
2. После наблюдаемого scheduled run заменить smoke на проверку всех targets и вернуть production
   cadence со смещением `:23`.
3. Сохранить ручной `workflow_dispatch` в отдельном workflow для восстановления и тестов.
4. Не добавлять автоматический retry после неуспешного обнаруженного release — это соответствует
   выбранной ручной политике восстановления.
5. На первом этапе не добавлять сбор задержек scheduled runs; при необходимости их можно будет
