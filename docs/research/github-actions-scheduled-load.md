...
Первоначально рассматривался запуск раз в два часа. После двух пропущенных GitHub scheduler окон
частота увеличена до одного запуска в час, чтобы сократить ожидание следующей попытки:
`23 * * * *`. Минута `:23`:
...
статистики. При текущем расписании запуск будет происходить в `00:23`,
`01:23`, ... UTC; GitHub также поддерживает явную IANA timezone в `schedule`.
